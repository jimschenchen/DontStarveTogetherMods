...
function DebugMenuScreen:OnBecomeActive()
	...
	for k,v in pairs(valid_recipes) do
		local tab = v.tab or RECIPETABS.MAGIC
		if type(tab) == "string" then	-- yeah, not sure what's up there
			tab = {str = tab}
		end
		craft_menus[tab] = craft_menus[tab] or {}
		table.insert(craft_menus[tab], menus.DoAction(v.name, function() for kk,vv in pairs(v.ingredients) do c_give(vv.type, vv.amount) end end))
	end
